...
# potentially to do for model causality
...
x = torch.tensor(df[['Temperature', 'Pressure', 'Relative Humidity', 'Wind Speed','DNI']].values, dtype=torch.float32)

class TransformerModel(nn.Module):
    def __init__(self, n_features, n_hiddens, n_layers, n_heads, dropout=0.01):
    ...
    def forward(self, src):
        src = torch.cat((torch.abs(torch.fft.fft(src,dim=0)),torch.tanh(src)))
        grk = src
...
criterion2 = nn.SmoothL1Loss(reduction='mean', beta=20)
...
for epoch in range(n_epochs):
    ...
    for i in range(0, len(x), batch_size):
        ...
        temp_1,temp_2,p_1,p_2,rh_2,rh_1,v_1,v_2,i_2,i_1 = batch[-2,0],batch[-1,0],batch[-2,1],batch[-1,1],batch[-2,2],batch[-1,2],batch[-2,3],batch[-1,3],batch[-2,4],batch[-1,4]
        ...
        t_pred = (mu - beta * i_pred - gamma * v_1 * r_pred - xi *(theta))**-1 + t_pred
        p_pred =torch.exp((mu - torch.log(t_pred/temp_1)) - 8.314*torch.log(p_1)) + p_pred
        r_pred = (v_pred*gamma)**-1 * (mu - alpha * t_pred **-1 - beta * i_1 - xi*theta) + r_pred
        v_pred = (r_pred*gamma)**-1 * (mu - alpha * t_pred **-1 - beta * i_1 - xi*(theta)) + v_pred 
        i_pred = beta ** -1 * (mu - alpha/t_pred - gamma * v_pred * r_pred - xi*theta) + i_pred 
        t_pred = (mu - beta * i_pred - gamma * v_1 * r_pred - xi *(theta))**-1 + t_pred
        p_pred =torch.exp((mu - torch.log(t_pred/temp_1)) - 8.314*torch.log(p_1)) + p_pred
        ...
        d_entropy_pt = torch.log(t_pred/temp_1)  +8.314* torch.log(p_pred/p_1) # the est delta entropy
        d_entropy_pt_a = torch.log(temp_2/temp_1)  +8.314* torch.log(p_2/p_1) #actusl
        ...
        loss_lcec =torch.abs(d_entropy_pt-d_entropy_pt_a)+ criterion3(x_prime[0:],batch[-1,0:])+((torch.abs(d_entropy_lce-d_entropy_pt_a))+torch.abs(d_entropy_lce-mu))+(torch.abs(mu-d_entropy_pt))+(torch.abs(theta - (rh_2 - rh_1)))#before there was no constant theta
        loss_lcec = torch.lgamma(loss_lcec)
        #now we can compute the temperature and add that to the final loss
        # Backward pass and optimization
        optimizer.zero_grad()

        loss =  (criterion2(x_prime[0:],batch[-1,0:]) ) #+ criterion(x_prime,batch[-1,])))# see if focusing more on temperature loss and less on sporadic losses like wind prediction helps, ie use the transformer to estimate the current state for nontemp variables
        ...
        loss = loss + loss_lcec + (criterion(x_prime[0:],batch[-1,0:]))
        loss.backward()
